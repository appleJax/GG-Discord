...
export default async function notifyGlobalPercentMilestone(msg, userId) {
  const totalCards = await tryCatch(Card.countDocuments().exec());
  ...
  for (const deckName of user.subScores) {
    deck = await tryCatch(Deck.findOne({ name: deckName }).lean().exec());
  ...
  if (currentPercentage > user.nextPercentMilestone) {
    msg.reply(
      `congratulations! 🌸 You just completed ${
        user.nextPercentMilestone * 100
      }% of all cards in the database!`
    );

    await tryCatch(
      User.updateOne({ userId }, { $inc: { nextPercentMilestone: 0.1 } })
        .exec()
        .catch(console.error)
    );
  }
